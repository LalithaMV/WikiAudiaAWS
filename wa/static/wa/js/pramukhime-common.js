...
function scriptChangeCallback(lang, kb, context) {
	...
	document.getElementById('cmdhelp').style.background = "transparent url('"+piresourcebase+"/static/wa/img/"+ icon.iconFile + "') " + (lang == 'english' ? 100 : -1 * icon.x) + "px " + (lang == 'english' ? 100 : -1 * icon.y) + "px no-repeat";
	...
	document.getElementById('pramukhimecharmap').src = piresourcebase +'/static/wa/img/' + pramukhIME.getHelpImage();
    var filename = pramukhIME.getHelp();
    if (filename != '') {
        //document.getElementById('pramukhimehelpdetailed').src = piresourcebase + 'help/' + filename;
		document.getElementById('pramukhimehelpdetailed').src =piresourcebase +'/static/wa/help/' + filename;
    }
