...
def pdfGen(Input,fontName,Output):
    ...
    log.setLevel(10)
    log.info("PDFGEN Entering pdf Gen : Input: "+Input+" : fontName+ "+fontName+" :Output : "+Output)
    try:
        pdf = FPDF()
    except:
        log.info("PDFGEN pdfGen Exception")
    log.info("PDFGEN one")
    pdf.add_page()
    log.info("PDFGEN two")
    ...
    log.info(ttfName)
    try:
	log.info("PDFGEN before add_font")
        pdf.add_font(fontName, '', ttfName, uni=True) 
	log.info("PDFGEN after add_font")
    except Exception,e:
	log.info("PDFGEN Reason : "+e)	
        log.info("PDFGEN adding font exception")
    try:
	log.info("PDFGEN before set_font")
        pdf.set_font(fontName, '', 14)
	log.info("PDFGEN after set_font")
    except Exception,e:
	log.info("PDFGEN Reason: "+e)
	log.info("PDFGEN setting font exception")
    log.info("PDFGEN font added and set")
    linestring = open(Input, 'r').read()
    log.info("three")
    pdf.write(8,linestring)
    pdf.ln(20)
    pdf.output(Output, 'F')
    #log.info("Exiting from pdf Gen")
